...
def connect_udp(sm, ip_range):
    ...
    if ip_version == 4:
        ...
        server_socket.sendto('None'.encode('utf-8'), (udp_ip, destination_port))
        recv_pk = server_socket.recv(1024).decode('utf-8')
        if recv_pk == 'None':
            recv_pk = server_socket.recv(1024).decode('utf-8')
        server_socket.sendto(public_key.encode('utf-8'), (udp_ip, destination_port))
        ...
        encrypted_data_b64 = base64.b64encode(encrypted_data)
        server_socket.sendto(encrypted_data_b64, (udp_ip, destination_port))
        ...
        server_socket2.bind(('', source_port))
        server_socket2.sendto('None'.encode('utf-8'), (udp_ip, destination_port, 0, 0))
        recv_pk = server_socket2.recv(1024).decode('utf-8')
        if recv_pk == 'None':
            recv_pk = server_socket2.recv(1024).decode('utf-8')
        server_socket2.sendto(public_key.encode('utf-8'), (udp_ip, destination_port, 0, 0))
        pk_node = rsa.PublicKey.load_pkcs1(base64.b64decode(recv_pk))
        full_msg = ''.join(random.choices(string.ascii_uppercase + string.digits, k=9)) + '\n' + sm
        encrypted_data = rsa.encrypt(full_msg.encode('utf-8'), pk_node)
        encrypted_data_b64 = base64.b64encode(encrypted_data)
        server_socket2.sendto(encrypted_data_b64, (udp_ip, destination_port, 0, 0))
