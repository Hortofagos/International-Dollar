...
def _entries_from_segment(data):
    ...
    for entry in entries:
        if not isinstance(entry, dict) or not {"leaf_index", "entry_hash", "submitted_at"}.issubset(entry):
...
def verify_archive(manifest_location, archive_base=None, operator_public_key=None, mirror=None, strict=False):
    ...
    try:
        hash_log_exporter.verify_manifest_signature(manifest, operator_public_key=operator_public_key)
    except Exception as exc:
    ...
    for segment in manifest["segments"]:
        ...
        if len(data) != int(segment["byte_length"]):
            raise ArchiveAuditVerificationError(f"segment byte length mismatch for {segment['path']}")
        segment_entries = _entries_from_segment(data)
        if not segment_entries:
            raise ArchiveAuditVerificationError(f"empty segment listed in manifest: {segment['path']}")
        if int(segment_entries[0]["leaf_index"]) != int(segment["first_leaf_index"]):
            raise ArchiveAuditVerificationError(f"segment first_leaf_index mismatch for {segment['path']}")
        if int(segment_entries[-1]["leaf_index"]) != int(segment["last_leaf_index"]):
            raise ArchiveAuditVerificationError(f"segment last_leaf_index mismatch for {segment['path']}")
        if len(segment_entries) != int(segment["entry_count"]):
            raise ArchiveAuditVerificationError(f"segment entry_count mismatch for {segment['path']}")
        if int(segment_entries[0]["leaf_index"]) != expected_leaf:
            raise ArchiveAuditVerificationError("archive segment ranges are not contiguous from leaf 0")
    ...
    if computed_root != signed_root["root_hash"]:
        raise ArchiveAuditVerificationError("archive entries do not produce the manifest signed root")
...
def main(argv=None):
    ...
    try:
        ...
        result = {"archive_valid": False, "mirror_cross_checked": bool(args.mirror), "error": str(exc)}
        if args.json:
            print(log_client.canonical_json(result))
        else:
