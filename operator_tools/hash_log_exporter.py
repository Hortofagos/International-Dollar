...
class OperatorEntrySource:
    ...
    def latest_root(self):
        with urllib.request.urlopen(f"{self.operator_url}/v1/root", timeout=self.timeout) as response:
...
def sign_manifest(manifest, operator_private_key):
    ...
    signed["signature"] = ind_token.b85_sign(operator_private_key, manifest_signature_payload(signed))
    return signed


def archive_id_for(log_id, signed_root_tree_size, signed_root_hash, segments, segment_hash_algorithm):
...
def verify_manifest_signature(manifest, operator_public_key=None):
    ...
    if public_key != signed_root["operator_public_key"].strip():
        raise HashLogExportError("archive manifest operator key does not match embedded signed root")
    if manifest["tree_algorithm"] != signed_root["tree_algorithm"]:
        raise HashLogExportError("archive manifest tree_algorithm does not match embedded signed root")
    if manifest["hash_algorithm"] != signed_root["hash_algorithm"]:
        raise HashLogExportError("archive manifest hash_algorithm does not match embedded signed root")
    if int(manifest["signed_root_tree_size"]) != int(signed_root["tree_size"]):
        raise HashLogExportError("archive manifest signed_root_tree_size does not match embedded signed root")
    if manifest["signed_root_hash"] != signed_root["root_hash"]:
        raise HashLogExportError("archive manifest signed_root_hash does not match embedded signed root")
    if int(manifest["signed_root_timestamp"]) != int(signed_root["timestamp"]):
        raise HashLogExportError("archive manifest signed_root_timestamp does not match embedded signed root")
    if int(manifest["archived_entry_count"]) != int(signed_root["tree_size"]):
        raise HashLogExportError("archive manifest entry count does not match signed root tree size")
    ...
    if manifest["archive_id"] != expected_archive_id:
        raise HashLogExportError("archive manifest archive_id does not match signed root and segments")
    if not ind_token.b85_verify(public_key, manifest["signature"], manifest_signature_payload(manifest)):
...
class StaticHashLogArchive:
    ...
    def write_manifest(self, signed_root, manifest_timestamp=None):
        ...
        atomic_write_text(self.archive_dir / "manifest.json", log_client.canonical_json(manifest) + "\n")
        return manifest
    ...
    def segment_manifest_entries(self, tree_size):
        ...
        for file_path in files:
            ...
            if last >= tree_size:
                raise HashLogExportError("hash log archive segment extends beyond signed root tree size")
            ...
            if expected == tree_size:
                break
        if expected != tree_size:
            raise HashLogExportError("hash log archive does not contain a complete prefix for signed root")
        return segments
...
def export_loop(source, archive, state_path, page_size=DEFAULT_PAGE_SIZE, poll_seconds=DEFAULT_POLL_SECONDS):
    while True:
...
def main():
    parser = argparse.ArgumentParser(description="Export the full IND transparency hash log for auditors")
    parser.add_argument("--operator-url", default=os.environ.get("IND_LOG_OPERATOR_URL", DEFAULT_OPERATOR_URL))
    parser.add_argument("--archive-dir", default=os.environ.get("IND_HASH_LOG_ARCHIVE_DIR", DEFAULT_ARCHIVE_DIR))
    parser.add_argument("--state-file", default=os.environ.get("IND_HASH_LOG_EXPORT_STATE", DEFAULT_STATE_FILE))
    parser.add_argument("--operator-private-key", default=os.environ.get("IND_LOG_OPERATOR_PRIVATE_KEY", ""))
    parser.add_argument("--operator-private-key-file", default=os.environ.get("IND_LOG_OPERATOR_PRIVATE_KEY_FILE", DEFAULT_PRIVATE_KEY_PATH))
    parser.add_argument("--operator-public-key", default=os.environ.get("IND_LOG_OPERATOR_PUBLIC_KEY", ""))
    parser.add_argument("--operator-public-key-file", default=os.environ.get("IND_LOG_OPERATOR_PUBLIC_KEY_FILE", DEFAULT_PUBLIC_KEY_PATH))
    parser.add_argument("--page-size", type=int, default=int(os.environ.get("IND_HASH_LOG_EXPORT_PAGE_SIZE", DEFAULT_PAGE_SIZE)))
    parser.add_argument("--poll-seconds", type=int, default=int(os.environ.get("IND_HASH_LOG_EXPORT_POLL_SECONDS", DEFAULT_POLL_SECONDS)))
    parser.add_argument("--timeout", type=int, default=30)
    parser.add_argument("--once", action="store_true")
    args = parser.parse_args()

    operator_private_key = args.operator_private_key or read_key_file(args.operator_private_key_file, "private_key")
    operator_public_key = args.operator_public_key or read_key_file(args.operator_public_key_file, "public_key")
    if not operator_private_key or not operator_public_key:
        print("ERROR: operator private and public keys are required to sign hash-log archive manifests", file=sys.stderr)
        raise SystemExit(2)
    ...
    if args.once:
        ...
        print(f"exported {exported} transfer-hash log entrie(s)")
        return
    export_loop(source, archive, args.state_file, page_size=args.page_size, poll_seconds=args.poll_seconds)
